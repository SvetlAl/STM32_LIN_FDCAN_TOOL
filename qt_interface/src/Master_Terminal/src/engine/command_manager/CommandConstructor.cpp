...
bool CommandConstructor::constructCmd(uint32_t _cmd, uint32_t address, uint32_t value32_to_string){
    m_cmd_output.clear();
    switch(_cmd){
    ...
    case ELP_DEVICE_SET_MODE:{
        ...
        elp_cmd.add_param(((address >> 8)   & ~0xFFFF00));
        elp_cmd.add_param(((address >> 0)   & ~0xFFFFFF00));
        value32_to_string &=~0xFFFFFF00;
        elp_cmd.add_param((uint8_t)value32_to_string);
        m_cmd_output.resize(elp_cmd.calculateStringSize());
        elp_cmd.makeString(m_cmd_output);
        return true;
        break;
