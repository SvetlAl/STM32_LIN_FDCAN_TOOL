...
Base {
    ...
    function try_to_set_value(_value){
        ...
        else if(!theVehicle.is_cruise_mlt_available){
            i_textfield.append_data("not supported\n");
            return;
        }
        if(_value > theVehicle.cruise_mlt){
            if(cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_CRUISE_MLT, _value)){
                i_centr_indctr.value_up();
            }
        }
        else if(_value < theVehicle.cruise_mlt){
            if(cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_CRUISE_MLT, _value)){
    ...
    MltButton{
        ...
        visible: theVehicle.is_cruise_mlt_available
    ...
    MltButton{
        ...
        function cmd(){
            try_to_set_value(theVehicle.cruise_mlt + 1);
        }
    }
    ...
    CentralButtonIndicator{
        ...
        st_value: theVehicle.is_cruise_mlt_available? theVehicle.cruise_mlt : 0
    }
    ...
    BaseTextField{
        ...
        st_text: theVehicle.is_cruise_mlt_available ? CoreImport.crs_clbr_available_description : CoreImport.crs_clbr_unavailable_description
    }
}
