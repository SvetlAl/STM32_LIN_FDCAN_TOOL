...
#include "USB_CDC/usb_cdc_desc.h"
...
static void enumerate_Reset(void);
static void enumerate_Setup(void);
// void initEndPoints(void); -> static

static void read_Fifo(uint8_t dfifo, uint16_t len);

/* Endpoint functions */
static uint32_t USB_CDC_setTxBuffer(uint8_t EPnum, uint8_t *txBuff, uint16_t len);

static uint32_t USB_CDC_transferRXCallback_EP0(uint32_t param);

static uint32_t USB_CDC_transmit_scheduler(void); /* this function monitors if any data is pending in circ buffer or whatever */
static uint32_t USB_FlushTxFifo(uint32_t EPnum, uint32_t timeout);
static uint32_t USB_FlushRxFifo(uint32_t timeout);
...
void USB_OTG_HS_init_device(){
	...
	*/
	...
	USB_EP_OUT(0)->DOEPTSIZ |= USB_OTG_DOEPTSIZ_STUPCNT;  /* STUPCNT==0x11 means, EP can receive 3 packets. RM says to set STUPCNT = 3*/
...
static void on_reset(){
	USB_FlushTxFifo(0x10, FLUSH_FIFO_TIMEOUT);
	...
	USB1_OTG_HS->GINTMSK = USB_OTG_GINTMSK_USBRST |
        ...
        USB_EP_IN(i)->DIEPINT = 0xFB7FU;
        USB_EP_IN(i)->DIEPCTL &= ~USB_OTG_DIEPCTL_STALL;
        USB_EP_OUT(i)->DOEPINT = 0xFB7FU;
        USB_EP_OUT(i)->DOEPCTL &= ~USB_OTG_DOEPCTL_STALL;
        USB_EP_OUT(i)->DOEPCTL |= USB_OTG_DOEPCTL_SNAK;}
...
uint32_t USB_FlushTxFifo(uint32_t EPnum, uint32_t timeout){
  ...
  } while ((USB1_OTG_HS->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0U);count = 0U;
  USB1_OTG_HS->GRSTCTL = (USB_OTG_GRSTCTL_TXFFLSH | (EPnum << 6));
	do{
    count++;
    if (count > timeout){
...
uint32_t USB_FlushRxFifo(uint32_t timeout){
  ...
  } while ((USB1_OTG_HS->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL) == 0U);
...
* brief  If you don't handle received data on OEPINT event, there is a risk that the data will be lost
...
/**
* brief  Perform some action with received data (EP0) and refresh EP buffer counter
...
* brief  Test function works as echo and shows number of received bytes
...
void enumerate_Reset(){
	...
	/* Flush All Fifos */
	USB_FlushTxFifo(0xF, FLUSH_FIFO_TIMEOUT); 
	USB_FlushRxFifo(FLUSH_FIFO_TIMEOUT); 
...
void OTG_HS_IRQHandler(){
	...
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_USBSUSP){  
		USB_CLEAR_INTERRUPT(USB_OTG_GINTSTS_USBSUSP);
		USB1_OTG_HS->GINTMSK = USB_OTG_GINTMSK_USBRST;
		return;
	 }
	if(USB1_OTG_HS->GINTSTS & USB_OTG_GINTSTS_ESUSP){  
		USB_CLEAR_INTERRUPT(USB_OTG_GINTSTS_ESUSP);
		USB1_OTG_HS->GINTMSK = USB_OTG_GINTMSK_USBRST;
		return;
	 }
	/**************************************************************/
	...
	/****************** 		EP 0 			*****************************/
		if( epnums & 0x00010000){ /* EndPoint INTerrupt bits correspond to EP0 OUT */  
			...
			if(epint & USB_OTG_DOEPINT_STUP){		/* Setup packet received */ 			
				enumerate_Setup();				
				}
...
uint32_t USB_CDC_send_array(uint8_t *txBuff, uint16_t len){
	if(len == 0) return EP_FAILED;
	uint16_t pos = 0;
	uint16_t data_to_send = len;
	
	while((data_to_send > MAX_CDC_EP1_TX_SIZ) && (pos < len )){
		USB_CDC_send_data(&txBuff[pos], MAX_CDC_EP1_TX_SIZ);
		data_to_send = (uint16_t)(data_to_send - MAX_CDC_EP1_TX_SIZ);
		pos = (uint16_t)(pos + MAX_CDC_EP1_TX_SIZ);
	}
	if(data_to_send == 0) return EP_OK;
	
	USB_CDC_send_data(&txBuff[pos], data_to_send);
	return EP_OK;
}
...
uint32_t USB_CDC_send_2data(uint8_t *txBuff, uint16_t len, uint8_t *txBuff2, uint16_t len2){	
	...
	if(get_circBuffer_freeSize() < (len + len2)) return EP_FAILED;
	else{
		write_to_circBuffer(txBuff, len);
		write_to_circBuffer(txBuff2, len2);
		return EP_OK;
	}
}
...
__WEAK uint32_t USB_CDC_receive_data(uint16_t length){
