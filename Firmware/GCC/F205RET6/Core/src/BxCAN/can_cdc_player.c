...
#include "BxCAN/can_cdc_player_buffer.h"

static can_injection_status theCanInjectionStatus = {0};
static dual_cdc_can_buff theDualBuffer;

static void setInjectionMsgTimer(can_injection_status *_status);
static void injectPresetMessage(can_injection_status *_status,eDeviceFunctionMode _canInjectionMode);
static void injection_timer_procedure(void);

static void calculate_tim_settings(can_injection_status *_status);
static void handle_buffer_overflow(void);
static void CDC_setInjectionMsg(can_injection_status *_status);

#ifdef SUPPORT_MEMCHIP_CAN_INJECTION
static void setInjectionMsg(can_injection_status *_status, eDeviceFunctionMode _canInjectionMode);
#endif // SUPPORT_MEMCHIP_CAN_INJECTION
...
static void injection_timer_procedure(){
...
//   ****************************************** START ***************************************      
//
//   CDC_can_inj_receive_data(uint8_t *data, uint32_t length) - fill the dual trace buffer
//   |
...
static void initMessageInjectionStatus(can_injection_status *_status){
