...
/************ Write ************/
static void write_can_msg (can_message *msg, CanBufferStruct* buf);

static void write_can1_buffer (can_message *msg);
static void write_can2_buffer (can_message *msg);
/************ Read ************/
static void read_can_msg(CanBufferStruct* buf,uint8_t mailbox_num);

static void read_can1_buffer(uint8_t mailbox_num);
static void read_can2_buffer(uint8_t mailbox_num);

/************ Freesize ************/
static uint32_t read_can_buf_free_size(CanBufferStruct* buf);	

static uint32_t read_can1_buf_free_size(void);	
static uint32_t read_can2_buf_free_size(void);	
...
static uint32_t read_can_buf_free_size(CanBufferStruct* buf){
	...
	uint32_t freeSz = (buf->write_pos > buf->read_pos) ? (uint32_t)(CAN_TX_BUFFER_SIZE - (uint32_t)(buf->write_pos - buf->read_pos)) :
