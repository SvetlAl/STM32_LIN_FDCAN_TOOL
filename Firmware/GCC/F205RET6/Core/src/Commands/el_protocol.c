...
static elp_cmd global_el_command; /* A global var to store a current cmd readings */

/*
static void elp_quadro_copy(el_quadro_buffer *qbuf_in, el_quadro_buffer *qbuf_out){
...
static uint32_t pick_cmd(elp_cmd_BufferAddress addr){
...
/**
* brief   Run through the received data in @elp_cmd_buffer and pick all the parameters
...
static uint32_t elp_parse_cmd(elp_cmd_BufferAddress addr){
	/****** pick a command *********/
...
/**
* brief  Check, if a command has been received
* brief  If a received string contains symbols @S and @E and something in between these symbols
* param   
* param 
...
/**
* brief  Write data into the cmd buffer and check, if a command has been received
* param  data
* param  length
* retval 
*/
...
/**
* brief  After some data is received look up through the buffer and try to find a command.
...
uint32_t find_and_parse_elp_cmd(){
	/*  Check, if a full string with both START and END symbols is received  */
	elp_cmd_BufferAddress cmdIndex = findElpCmd();
	
	/********** Amount of received bytes is enough to have a cmd  ********/
