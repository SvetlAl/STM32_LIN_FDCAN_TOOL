...
*                                                                                                                                  then the USART transmission is done in "lin_process_engine()" in the main loop
*
*
*  Master RX procedure is processed in USARTX_IRQHandler. If Master is waiting for slave response, incoming data is written into RX buffer, and then processed in "lin_process_engine()" in the main loop while(1)
*
*
*
*
...
*                                                            ==> if a header is received, "__WEAK void slave_device_rx()" function processes the data. In device demonstration mode
*                                                                Slave ignores all id's exept 0x1A, 0x2A and 0x3A. If the id 0x1A and 0x2A received, Slave sends back a dummy response.
*                                                                For example. if the id is 0x3A, It keeps receiveing data with a flag LIN_STATUS_WAIT_DATA, after all 9 bytes are received, a flag LIN_STATUS_READ_DATA is set
*                                                                and then this data is processed in "lin_process_engine()" in the main loop while(1).
*
*                                                                In the first place, a message is scheduled in IRQ,
*                                                                then the USART transmission is done in "lin_process_engine()" in the main loop
*
...
*  
*  
*  GatewayLin.SlaveLin is supposed to receive all data coming from a target netwotk (Master) and re-transmit it through GatewayLin.MasterLin.
*  GatewayLin.MasterLin is supposed to receive responses coming from a target netwotk (Slave).
*
*  Slave RX procedure is processed in USARTX_IRQHandler.
*                                                        After consequently a LIN break and then a header received, the header is re-transmitted through GatewayLin.MasterLin immediately.
...
*                                                        In case this is a data message, a completely received message is processed in "lin_process_engine()" in the main loop while(1).
...
//== void lin_process_engine() - general processing function in the main loop =
typedef enum{
	VALID_OK = 0,
...
void TIM3_IRQHandler(void){
	if(TIM3->SR & TIM_SR_UIF){
		...
		// If master hasn't received response yet, reset status and continue polling.
...
#if ((defined(SUPPORT_LIN)) && (LIN_MASTER_USART_NUM == 2))
...
void USART2_IRQHandler(void){
	...
	if (USART2->SR & USART_SR_LBD){
		...
		volatile uint16_t temp_receivedByte = (uint16_t)(USART3->DR); // Clear LIN break received bits
		USART2->SR &= ~USART_SR_RXNE;
		(void)temp_receivedByte;
 ...
 //=============================================================================
 //====================== Mater receives an incoming byte ======================
 //=============================================================================
...
//		if(IS_LIN_MODE(LIN_MODE_DEVICE)){
		...
		if(MASTER.status == LIN_STATUS_WAIT_DATA){
			if(MASTER.rx_cnt!=0){
				MASTER._usart_rx_buffer->write(&receivedByte, 1);
		...
		// The "SEND SYNC" or "SEND HEADER" statuses mean, that MASTER must ignore its own SYNC and ID 
			...
			if(receivedByte == LIN_SYNC_VAL) set_master_status(LIN_STATUS_SEND_HDR); 
...
__WEAK void master_poll(){
	...
	// and set receive counter according to byte count incorporated in id
	// 0x00-0x1F - 2 bytes
	// 0x20-0x2F - 4 bytes
	// 0x30-0x3F - 8 bytes
	if(IS_LIN_MODE(LIN_MODE_DEVICE)){
...
#if ((defined(SUPPORT_LIN)) && (LIN_SLAVE_USART_NUM == 3))
...
void USART3_IRQHandler(void){
  ...
  //=============================================================================
	...
	//=============================================================================
  //====================== Slave receives an incoming byte ======================
  //=============================================================================
    ...
    //=============================================================================
		...
		//=============================================================================
    //======================  Slave receives the SYNC bits  =======================
    //=============================================================================
		if(SLAVE.status == LIN_STATUS_WAIT_SYNC){
			if(receivedByte == LIN_SYNC_VAL){
   ...
   //=========================== A header received ===============================
   //=============================================================================
		if(SLAVE.status == LIN_STATUS_WAIT_HDR){       // If a header received
			LinFrameHeader rec_header;
			rec_header.header = receivedByte;
      ...
      /******************   @DEVICE   ******************/
			...
			/******************   @GATEWAY   *****************/
				...
				// re-transmit a header, then be ready to receive data bytes (In case this is a data message)
				
				// Since this is unknown what network (Master or Slave) will send data, set receive mode for both (Master and Slave) 
   ...
   //=============================================================================
		...
		else if(SLAVE.status == LIN_STATUS_WAIT_DATA){       // If data byte received		
			
			if(SLAVE.rx_cnt!=0){
				SLAVE._usart_rx_buffer->write(&receivedByte, 1);
...
*/
static void init_lin_instances(uint16_t poll_period){
	#if defined(SUPPORT_LIN_MASTER)
	gatewayLIN.masterLIN.usart_num = LIN_MASTER_USART_NUM;
	gatewayLIN.masterLIN.usart_ptr = LIN_MASTER_USART_PERIPH;
	usart_rx_buffer* master_buf = 0;
	usart_get_rx_buf_ptr(LIN_MASTER_USART_NUM, master_buf);
	gatewayLIN.masterLIN._usart_rx_buffer = master_buf;
	#endif
	
	#if defined(SUPPORT_LIN_SLAVE)
	gatewayLIN.slaveLIN.usart_num = LIN_SLAVE_USART_NUM;
	gatewayLIN.slaveLIN.usart_ptr = LIN_SLAVE_USART_PERIPH;
	usart_rx_buffer* slave_buf = 0;
	usart_get_rx_buf_ptr(LIN_SLAVE_USART_NUM, slave_buf);
	gatewayLIN.slaveLIN._usart_rx_buffer = slave_buf;
...
void start_master_polling(uint16_t poll_period){
	startBasicTimer(CAN_INJ_TIM_PSC, poll_period, LIN_DEVICE_TIM);
}
