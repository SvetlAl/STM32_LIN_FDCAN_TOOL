...
static uint32_t USB_CDC_transferTXCallback(uint8_t EPnum);
...
static void write_to_circBuffer(uint8_t *buf, uint16_t len){
	...
	if((writePtrTxCbuf + len) >= CIRC_BUFFER_TX_SIZE){
		...
		writePtrTxCbuf =  (uint16_t)((uint16_t)(writePtrTxCbuf + len) - (uint16_t)CIRC_BUFFER_TX_SIZE);
	}
	else{
...
static uint32_t USB_CDC_transmit_scheduler(){
