...
#include "../bus/FDCAN_injection/FDCAN_injection_message.h"
...
public:
    ...
    Q_INVOKABLE bool readMemoryData(int address, int transaction_length){
    #ifndef ANDROID_V
        const int max_rx_bytes = m_pCDC_drv->maxRxBytes_per_request();
        int temp_length = transaction_length;
        int addr_counter = address;
        int fail_counter = transaction_length/64;
        ...
        while(temp_length > max_rx_bytes){
            ...
            else m_cmdConstructor.constructCmd(ELP_CMD_SPI_MEMCHIP_READ32, addr_counter, max_rx_bytes);

            exchange_cmd_string_HEX(m_cmdConstructor.cmd_string(), &buffer, false, 10);
            if(buffer.size() == max_rx_bytes){
                bool result = m_pMemoryManager->parseByteArray(addr_counter, buffer, max_rx_bytes);
                if(!result){
         //           qDebug() << "buffer.size " << buffer.size();
         //           qDebug() << "max_rx_bytes " << max_rx_bytes;
                    return false;
                }
   ...
   //             qDebug() << "failed pos " << temp_length;
            }
            buffer.clear();
            int passed_bytes = addr_counter - address;
            const int percent = transaction_length/100;
            int passed_percent = passed_bytes/percent;

            emit memoryReadProgressSend(QString::number(passed_percent) + "%");
 //           qDebug() << "addr_counter " << addr_counter;
 //           qDebug() << QString::number(passed_percent);
        }
        ...
        while(temp_length > 0){
            ...
            else m_cmdConstructor.constructCmd(ELP_CMD_SPI_MEMCHIP_READ32, addr_counter, temp_length);
            exchange_cmd_string_HEX(m_cmdConstructor.cmd_string(), &buffer, false, 100);
            if(buffer.size() == temp_length){
    ...
    /********************************** FD CAN ************************************************/
    Q_INVOKABLE void read_fdcan_settings();
    Q_INVOKABLE bool write_fdcan_settings();
    Q_INVOKABLE bool restart_fdcan();
    Q_INVOKABLE bool fdcan_send_single_msg(int can_num);
    Q_INVOKABLE bool save_FDCAN_settings();
    ...
    /********************************** FD CAN ************************************************/
    Q_INVOKABLE bool fdcan_updateOverrideStatus();
    Q_INVOKABLE bool fdcan_sendOverrideFilterConfiguration();
    ...
    //=================================
    FDCAN_injection_message *pFdcan_inj_msg() const;
    void setPFdcan_inj_msg(FDCAN_injection_message *PFdcan_inj_msg);
...
signals:
    void memoryReadProgressSend(const QString _status);
...
private:
    ...
    FDCAN_injection_message *m_pFdcan_inj_msg = nullptr;
