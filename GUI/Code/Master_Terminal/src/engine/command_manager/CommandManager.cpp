...
void CommandManager::start_asynch_read(int mode){
    ...
    else if(mode == CDC_Driver::AsynchReadToFDScanner){
        QObject::connect(m_pCDC_drv, &CDC_Driver::async_data_passover, this, &CommandManager::onCanScannerDataRecieved);
        QObject::connect(this, &CommandManager::asynchronousDataIn, m_pBusManager, &BusManager::fd_processIncomingData);
    }
...
void CommandManager::stop_asynch_read(int mode){
    ...
    if(mode == CDC_Driver::AsynchReadToScanner){
        ...
        QObject::disconnect(m_pCDC_drv, &CDC_Driver::async_data_passover, this, &CommandManager::onCanScannerDataRecieved);
        QObject::disconnect(this, &CommandManager::asynchronousDataIn, m_pBusManager, &BusManager::fd_processIncomingData);
    }
};
...
bool CommandManager::writeRawData(uint32_t address, const QByteArray &data, bool append){
    int transaction_length = data.size();
    if(transaction_length == 0) return true;
    if(m_pDeviceManager == nullptr) return false;
    uint8_t option = (append) ? 0x00 : 0xFF;

    //qDebug() << "data.size() " << data.size();

    if((address <= 0xFFFFFF) && (transaction_length <= 0xFFFFFF)){
         m_cmdConstructor.constructCmd(ELPR_CMD_SPI_MEMCHIP_PROGRAMM_CHIP, address, data.size(), option);
    }
    else m_cmdConstructor.constructCmd(ELPR_CMD_SPI_MEMCHIP_PROGRAMM_CHIP32, address, data.size(), option);

    qDebug() << "m_cmdConstructor.cmd_string() " << m_cmdConstructor.cmd_string();
    //return true;
    ...
    if(m_pDeviceManager->isMemchipNand()) memory_chip_timeout = DeviceManager::nand_write_timeout;
...
/********************************** FD CAN ************************************************/
void CommandManager::read_fdcan_settings(){
    if(!isConnected()) return;
    m_cmdConstructor.constructCmd(ELPR_FDCAN_GET_SETTINGS);
    QByteArray response;
    exchange_cmd_string_HEX(m_cmdConstructor.cmd_string(), &response);
    m_pDeviceManager->refresh_fdcan_settings(response);
};

bool CommandManager::save_FDCAN_settings(){
    m_cmdConstructor.constructCmd(ELP_SAVE_FDCAN_STNGS);
    QString response = exchange_cmd_string(QString::fromUtf8(m_cmdConstructor.cmd_string()));
    if(!CommandConstructor::isResponseOk(response)){
        return false;
    }
...
bool CommandManager::write_fdcan_settings(){
    if(!isConnected()) return false;
    if(m_pDeviceManager == nullptr) return false;
    m_cmdConstructor.constructCmd(ELP_UPDATE_FDCAN_STNGS);

    const QByteArray data = m_pDeviceManager->read_fdcan_settings();
    const int timeout = data.size()/16;

    QByteArray response;
    exchange_cmd_string_HEX(m_cmdConstructor.cmd_string(), &response, false, timeout);
    ...
    response = waitAndReadResponse(timeout);
    if(!CommandConstructor::isResponseOk(QString::fromUtf8(response))){
        return false;
    }
    else{
...
bool CommandManager::restart_fdcan(){
    m_cmdConstructor.constructCmd(ELP_RESTART_FDCAN);
    QString response = exchange_cmd_string(QString::fromUtf8(m_cmdConstructor.cmd_string()));
    if(!CommandConstructor::isResponseOk(response)){
        return false;
    }
...
static inline uint32_t fdcan_construct_msg_from_qml(uint8_t _flags, uint8_t _dlc, const QString &_id, const QString &_data, uint8_t* _raw_data){
    u_fdcan_message_hdr *input_msg_hdr = (u_fdcan_message_hdr*)_raw_data;
    input_msg_hdr->msg_hdr.dlc = _dlc;
    input_msg_hdr->msg_hdr.fdflags.byte = _flags;
    bool conversion_result = 0;
    uint32_t int_id = _id.toInt(&conversion_result, 16);
    if(!conversion_result) return 0;
    input_msg_hdr->msg_hdr.id = int_id;
    int pos = sizeof(fdcan_message_hdr);

    const fdcan_message_hdr *msg_hdr = &input_msg_hdr->msg_hdr;
    int total_msg_sz = FDCAN_FD_MSG_LENGTH(msg_hdr);
    for(size_t i = 0; i < total_msg_sz; i++){
        uint8_t msg_dat_value = _data.mid(i*4, 4).toInt(&conversion_result, 16);
        if(!conversion_result) return false;
        _raw_data[pos++] = msg_dat_value;
    }
    return total_msg_sz + sizeof(fdcan_message_hdr);
}


bool CommandManager::fdcan_send_single_msg(int can_num){
    bool result = false;
    QByteArray msg_bytes;
    msg_bytes.resize(sizeof(fdcan_message_hdr) + 64); // max possible size
    const char* rawData = msg_bytes.constData();
    uint8_t* raw_pointer = reinterpret_cast<uint8_t*>(const_cast<char*>(rawData));

    uint16_t data_siz = fdcan_construct_msg_from_qml(pFdcan_inj_msg()->makeFlags(), pFdcan_inj_msg()->dlc(),
                                                     pFdcan_inj_msg()->id(), pFdcan_inj_msg()->makeData(), raw_pointer);

    if(data_siz == 0) return false;
    msg_bytes.resize(data_siz); //

    /*
    for(uint32_t i =0; i < data_siz; i++){
        qDebug() << raw_pointer[i];
    }
    */

    if(!isConnected()) return false;
    if(m_pDeviceManager == nullptr) return false;
    m_cmdConstructor.constructCmd(ELPR_SEND_CUSTOM_FDCAN_MSG, (uint32_t)data_siz, (uint32_t)can_num);
    const int timeout = msg_bytes.size()/16;
    ...
    int max_tx_bytes = m_pCDC_drv->maxTxBytes_per_request();
    int temp_length = msg_bytes.size();
    int pos = 0;

    while(temp_length >= max_tx_bytes){
        sendRawData(msg_bytes.mid(pos, max_tx_bytes));
        temp_length -= max_tx_bytes;
        pos += max_tx_bytes;
    }

    if((temp_length != 0) & (temp_length < max_tx_bytes)){
        sendRawData(msg_bytes.mid(pos, temp_length));
    }
...
bool CommandManager::fdcan_updateOverrideStatus(){
    if(!isConnected()) return false;
    m_cmdConstructor.constructCmd(ELPR_OVR_FLTR_GET_CURRENT_STATUS);
    sendRawData(m_cmdConstructor.cmd_string());
    QByteArray ovr_status = waitAndReadResponse(1000);
    ...
    int trials = 10;
    while(trials-- & (ovr_status.size() < DEVICE_FDCAN_OVERRIDE_FILTER_SETTINGS_LENGTH) ){
        QByteArray part = waitAndReadResponse(1000);
        ovr_status.append(part);
    }
    if(ovr_status.size() == DEVICE_FDCAN_OVERRIDE_FILTER_SETTINGS_LENGTH){
        m_pDeviceManager->refreshFdcanOverrideStatus(ovr_status);
        return true;
    }
    return false;
}
...
bool CommandManager::fdcan_sendOverrideFilterConfiguration(){
    if(!isConnected()) return false;
    if(m_pDeviceManager == nullptr) return false;
    m_cmdConstructor.constructCmd(ELP_UPDATE_OVR_FILTERS);
    const QByteArray data = m_pDeviceManager->getFdcanOvrFilterData();
...
FDCAN_injection_message *CommandManager::pFdcan_inj_msg() const{
    return m_pFdcan_inj_msg;
}
void CommandManager::setPFdcan_inj_msg(FDCAN_injection_message *PFdcan_inj_msg){
    m_pFdcan_inj_msg = PFdcan_inj_msg;
}
