...
public:
 ...
 /*
    ...
    */
    ...
    /**
    * brief  Make a string out of FD CAN data
    * param data array
    * param actual length
    * retval Rich text QString
    */
    static QString FDCAN_convertByteArrayToFormattedString(const QByteArray& m_data_array, int msg_length) {
        QString result;
        const int step = 8;
        const int total_msg_length = m_data_array.size() >= msg_length ? msg_length : m_data_array.size();
        const int last_pos = total_msg_length - 1;

        for (int i = 0; i < total_msg_length; i += step) {

            int residue = total_msg_length - i;
            int pack_size = residue >= step ? step : residue;

            for (int j = 0; j < pack_size; j++) {
                result += QString("%1").arg(static_cast<uchar>(m_data_array.at(i + j)), 2, 16, QChar('0')).toUpper();

                if(i + j == last_pos) return result;
 ...
 *         response.append("<font color=\"#FF0000\">");
    ...
    */
    ...
    static int can_timestamp_to_intTime(const QString& timestamp) {
        int dotIndex = timestamp.indexOf(".");

        if (dotIndex == -1 || timestamp.count(".") > 1) {
            // Invalid input format
            return -1;
        }

        QString secondsStr = timestamp.left(dotIndex);
        QString milisecondsStr = timestamp.mid(dotIndex + 1);
        ...
        return (newSeconds*10000 + newMiliseconds);
    }
