...
#define DEVICE_FDCAN_OVERRIDE_FILTER_SETTINGS_LENGTH	sizeof(fdcan_filter)

class FDCAN_OverrideStatus : public QObject{

public:
    explicit FDCAN_OverrideStatus(QObject *parent = 0) : QObject(parent){
    }
    ~FDCAN_OverrideStatus() {};
    friend class DeviceManager;
    ...
    //=========================== Class setters/getters ===============================
    const QByteArray getRawData() const;
    u_fdcan_filter* get_main_ptr(){return &m_ovr_status;}

private:
    u_fdcan_filter m_ovr_status;
};
