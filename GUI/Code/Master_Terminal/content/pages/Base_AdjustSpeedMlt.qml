...
Base {
    ...
    function try_to_set_value(_value){
        ...
        else if(!theVehicle.is_speedometer_mlt_available){
            i_textfield.append_data("not supported\n");
            return;
        }
        if(_value > theVehicle.speedometer_mlt){
            if(cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_SPEEDOMETER_MLT, _value)){
                i_centr_indctr.value_up();
            }
        }
        else if(_value < theVehicle.speedometer_mlt){
            if(cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_SPEEDOMETER_MLT, _value)){
    ...
    MltButton{
        ...
        visible: theVehicle.is_speedometer_mlt_available
    ...
    MltButton{
        ...
        function cmd(){
            try_to_set_value(theVehicle.speedometer_mlt + 1);
        }
    }
    ...
    readonly property int st_compostition_center_y: (i_save_button.st_button_center_y + i_device_hw_button.st_button_center_y) / 2
    readonly property int st_distance_from_UpBtm_to_LwrTop: ((i_device_hw_button.y - i_device_hw_button.height ) - i_save_button.y )
    ...
    CentralButtonIndicator{
        ...
        st_value: theVehicle.is_speedometer_mlt_available ? theVehicle.speedometer_mlt : 0
    }
    ...
    BaseTextField{
        id: i_textfield
        y: (i_device_hw_button.y + i_device_hw_button.height + vertical_gap)
        height: freeMainframeSpace - y
        width: root.width * st_side_crop
        anchors.horizontalCenter: root.horizontalCenter
        st_text: theVehicle.is_speedometer_mlt_available ? CoreImport.spd_clbr_available_description : theVehicle.model == 0x0002 /* Hilux */ ?
        CoreImport.spd_clbr_pulse_available_description : CoreImport.spd_clbr_unavailable_description
    }
}
