import QtQuick
import QtQuick.Controls
import QtQuick.Layouts
import CoreImport;
import "BaseItems";
import "SimpleItems";

Base {
    id: root

    /********* size settings ********/
    property real buttons_width_mlt: 3.5
    property int width_grid_sz: width*st_side_crop / 10
    /********* colors settings ********/

    /********* values ********/

    property int composition_side_crop: (width - (width * st_side_crop))/2
    property int vertical_gap: height/20

    /********* alias ********/


    /********* translated ********/
    property string st_can1_anno: qsTr("CAN 1")
    property string st_can2_anno: qsTr("CAN 2")

    property string st_save_button_text: qsTr("Save")
    property string st_refresh_button_text: qsTr("Refresh")
    property string st_apply_button_text: qsTr("Apply")
    property string st_restart_button_text: qsTr("Restart")


    function retranslateUi(){
        st_can1_anno = qsTr("CAN 1");
        st_can2_anno = qsTr("CAN 2");

        st_save_button_text = qsTr("Save");
        st_refresh_button_text = qsTr("Refresh");
        st_apply_button_text =  qsTr("Apply");
        st_restart_button_text =  qsTr("Restart");
    }
    Connections {
        target: qmlTranslator  /* registered in root context */
        function onLanguageChanged(){
            retranslateUi();
        }
    }
    Component.onCompleted: {
        retranslateUi();
    }
    Component.onDestruction: {
    }


    property int current_page: 0;
    function setLoaderItem(page){
        current_page = page;
        if(page === 0){
            i_content_loader.sourceComponent = i_can1_id
        }
        else if( page === 1){
            i_content_loader.sourceComponent =  i_can2_id
        }
        else i_content_loader.sourceComponent =  i_can1_id
    }

    property int hor_space: width - (composition_side_crop*2)
    property int hor_item_gap: composition_side_crop
    property int hor_space_step: (hor_space/4)
    property int hor_item_width: hor_space_step - composition_side_crop

    property bool changes_committed: false

    MltButton{
        id: i_save_button
        x: root.hor_item_gap + (root.hor_space_step*0)
        width: hor_item_width

        y: vertical_gap/4
        height: st_initial_item_height
        st_button_mode: (interface_console.connection_status_description[0] === "Connected") ? true : false

        st_text: st_save_button_text
        st_normal_color: st_button_mode ? st_G_color : st_N_color
        function cmd(){
            root.forceActiveFocus();
            cmd_manager.save_FDCAN_settings();
        }
    }
    MltButton{
        id: i_refresh_button
        x: root.hor_item_gap + (root.hor_space_step*1)
        width: hor_item_width

        y: i_save_button.y
        height: st_initial_item_height

        st_button_mode: (interface_console.connection_status_description[0] === "Connected") ? true : false
        st_text: st_refresh_button_text
        st_normal_color: st_button_mode ? st_G_color : st_N_color
        function cmd(){
            root.forceActiveFocus();
            cmd_manager.read_fdcan_settings();
        }
    }

    MltButton{
        id: i_apply_button
        x: root.hor_item_gap + (root.hor_space_step*2)
        width: hor_item_width

        y: i_save_button.y
        height: st_initial_item_height

        st_button_mode: (interface_console.connection_status_description[0] === "Connected" && changes_committed === true) ? true : false
        st_text: st_apply_button_text
        st_normal_color: st_button_mode ? st_R_color : st_N_color
        function cmd(){
            root.forceActiveFocus();
            cmd_manager.write_fdcan_settings();
            changes_committed = false;
        }
    }
    MltButton{
        id: i_restart_button
        x: root.hor_item_gap + (root.hor_space_step*3)
        width: hor_item_width

        y: i_save_button.y
        height: st_initial_item_height

        st_button_mode: (interface_console.connection_status_description[0] === "Connected") ? true : false
        st_text: st_restart_button_text
        st_normal_color: st_button_mode ? st_G_color : st_N_color
        function cmd(){
            root.forceActiveFocus();
            cmd_manager.restart_fdcan();
        }
    }
    MltButton{
        id: i_can1_button
        x: i_save_button.x
        y: i_save_button.y + i_save_button.height + (vertical_gap/4)

        height: st_initial_item_height
        width: i_refresh_button.x + i_refresh_button.width - i_save_button.x
        st_button_mode: true

        st_text: st_can1_anno
        st_normal_color: current_page === 0 ? st_G_color : st_N_color
        function cmd(){
            root.forceActiveFocus();
            setLoaderItem(0);
        }
    }

    MltButton{
        id: i_can2_button
        x: i_apply_button.x
        y: i_can1_button.y

        height: st_initial_item_height
        width: i_can1_button.width
        st_button_mode: true

        st_text: st_can2_anno
        st_normal_color: current_page === 1 ? st_G_color : st_N_color
        function cmd(){
            root.forceActiveFocus();
            setLoaderItem(1);
        }
    }

    Flickable {
        id: i_flickable
        width: root.width
        y: (i_can1_button.y + i_can1_button.height + (vertical_gap/4))
        height: freeMainframeSpace - y
        contentHeight: i_content_loader.height
        clip: true
        flickDeceleration  : 1500.00
        maximumFlickVelocity : 1500.00
        ScrollBar.vertical:
            ScrollBar {
            policy: ScrollBar.AlwaysOn
        }

        Loader {
            id: i_content_loader
            y: 0
            x: 0
            width: root.width
            height: i_can1_id.height
            sourceComponent: i_can1_id
            onLoaded: function(){
            }
        }
    }

    Connections {
        target: i_content_loader.item
        function onSendHeight(msg) {
            i_content_loader.height = msg;
        }
        function onChangesCommitted() {
            changes_committed = true;
        }
    }

    Component {
        id: i_can1_id
        Component_Base_FD_Settings_CAN1{
            width: i_content_loader.width
            height: i_content_loader.height
            Component.onCompleted: {
            }
        }
    }

    Component {
        id: i_can2_id
        Component_Base_FD_Settings_CAN2{
            width: i_content_loader.width
            height: i_content_loader.height
            Component.onCompleted: {
            }
        }
    }

}
