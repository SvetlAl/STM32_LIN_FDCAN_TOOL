...
Rectangle {
    ...
    property string st_textfield_anno: "LIN id"
    property string st_master_to_slave_anno: "Master to Slave"
    property string st_slave_to_master_anno: "Slave to Master"
    property string st_edit_anno: "Edit"
    property string st_ok_anno: "OK"
    ...
    property bool st_dir_mosi: true
    ...
    function onChangesCommit(dir_mosi){
        cmd_manager.set_LIN_filter(dir_mosi);
    }
    function onEditStarted(dir_mosi){
        cmd_manager.get_LIN_filter(dir_mosi);
    }
...
*/
    ...
    MltButton{
        ...
        st_text: st_dir_mosi === true ? st_master_to_slave_anno : st_slave_to_master_anno
        st_normal_color: st_dir_mosi === true ? st_B_color :st_Y_color
        function cmd(){
        }
    }
    ...
    MltButton{
        id: i_edit_button
        x: i_direction_button.x + i_direction_button.width + st_side_gap/2
        anchors.bottom: i_id.bottom
        height: st_total_height
        width: st_side_gap*4
        st_button_mode: true
    ...
    MltButton{
        ...
        st_text: st_ok_anno
        st_normal_color: changes_commited ? st_Y_color : st_N_color
        function cmd(){
            if(st_editable){
                //st_edit_mode = !st_edit_mode
                onChangesCommit(st_dir_mosi)
