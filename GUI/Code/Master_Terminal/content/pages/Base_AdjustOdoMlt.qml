...
Base {
    ...
    property string st_freeze_button_text: qsTr("Freeze")
    ...
    function try_to_set_value(_value){
        ...
        else if(!theVehicle.is_odometer_mlt_available){
            i_textfield.append_data("not supported\n");
            return;
        }
        if(_value > theVehicle.odometer_mlt){
            if(cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_ODOMETER_MLT, _value)){
                i_centr_indctr.value_up();
            }
        }
        else if(_value < theVehicle.odometer_mlt){
            if(cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_ODOMETER_MLT, _value)){
    ...
    function retranslateUi(){
        ...
        st_freeze_button_text = qsTr("Freeze");
    ...
    MltButton{
        ...
        visible: theVehicle.is_odometer_mlt_available
    ...
    MltButton{
        ...
        function cmd(){
            try_to_set_value(theVehicle.odometer_mlt + 1);
        }
    }
    ...
    MltButton{
        ...
        readonly property int st_button_center_y: y + height/2
        visible: theVehicle.is_odometer_mlt_available
        st_text: st_freeze_button_text

        st_checkbox_mode: true
        st_checkbox_value: theVehicle.isOdoFreeze
        function chkbx_cmd() {
            if(!(cmd_manager.isConnected)) return;
            if(theVehicle.isOdoFreeze){
                cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_MODE, CoreImport._ELP_VEHICLE_RESET_ODO_FREEZE_MSK);
                cmd_manager.updateVehicleStatus();
            }
            else{
                cmd_manager.setCalibratorValue(CoreImport._ELP_VEHICLE_SET_MODE, CoreImport._ELP_VEHICLE_SET_ODO_FREEZE_MSK);
                cmd_manager.updateVehicleStatus();
            }
        }
    ...
    CentralButtonIndicator{
        ...
        st_value: theVehicle.is_odometer_mlt_available? theVehicle.odometer_mlt : 0
    }
    ...
    BaseTextField{
        ...
        st_text: theVehicle.is_odometer_mlt_available ? CoreImport.odo_clbr_available_description : theVehicle.model == 0x0002 /* Hilux */ ?
        CoreImport.odo_clbr_pulse_available_description : CoreImport.odo_clbr_unavailable_description
    }
}
